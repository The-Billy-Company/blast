...
The claim is that exact match and compression answer better together than
...
Oracles come from independent `gist.files` searches and from the fixture's own
...
import gist
import relate
from blast import radius
from blast import verbs as compose


def _binary_available() -> bool:
    if shutil.which("blast") is not None:
        return True
    try:
        blast.binary()
    except blast.GistNotFoundError:
        return False
    return True
...
def test_blast_finds_the_definition_and_its_caller(corpus):
    ...
    oracle = {p.removeprefix("./") for p in gist.files("reconcile_ledger", paths=["."], cwd=corpus)}
...
def test_a_narrowed_pack_excludes_the_file_that_matches_no_pattern(corpus):
    ...
    packed = relate.pack(query, roots=["."], top=4, cwd=corpus)
    assert any(p.endswith("decoy.md") for p in packed.paths), "fixture no longer tests anything"

    picks = relate.pack(query, matching=["reconcile_ledger"], roots=["."], top=4, cwd=corpus)
...
def test_match_all_narrows_further_than_match_any(corpus):
    ...
    any_hit = relate.pack(query, matching=both, match="any", roots=["."], cwd=corpus)
    all_hit = relate.pack(query, matching=both, match="all", roots=["."], cwd=corpus)
...
def test_narrowed_recall_prices_only_the_matching_files(corpus):
    ...
    query = "settle every entry against one charge identifier"
    wide = {r.path.removeprefix("./") for r in relate.recall(query, roots=["."], cwd=corpus)}
    assert "decoy.md" in wide, "fixture no longer tests anything"
    narrow = relate.recall(query, matching=["reconcile_ledger"], roots=["."], cwd=corpus)
...
def test_narrowed_families_group_the_renamed_twin_file_kinship_cannot_see(corpus):
    whole_file = relate.pairs(channel="copies", roots=["."], max_distance=0.25, cwd=corpus)
...
def test_the_narrowed_complement_reports_what_grouped_with_nothing(corpus):
    ...
    assert not list(relate.families(**axes))
    lonely = relate.distinct(**axes)
