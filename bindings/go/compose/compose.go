// Package compose is the plane where both engines answer one question:
...
func (c *Corpus) rows(ctx context.Context, op analytic.Op, p analytic.Compose, scoped bool) (*runtime.Rows, error) {
...
func (c *Corpus) Context(ctx context.Context, p analytic.Compose) ([]relate.Pick, error) {
	rows, err := c.rows(ctx, analytic.OpContext, p, true)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (c *Corpus) Family(ctx context.Context, p analytic.Compose) ([]relate.Family, error) {
	rows, err := c.rows(ctx, analytic.OpFamily, p, true)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (c *Corpus) Provenance(ctx context.Context, p analytic.Compose) ([]Attribution, error) {
	rows, err := c.rows(ctx, analytic.OpProvenance, p, false)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (c *Corpus) Blast(ctx context.Context, p analytic.Compose) (Blast, error) {
	rows, err := c.rows(ctx, analytic.OpBlast, p, false)
	if err != nil {
		return Blast{}, err
	}
	defer rows.Close()
