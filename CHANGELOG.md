...
- - **`cargo publish` reaches crates.io again.** Both 1.1 releases tagged, built,
    ...
    sibling `irgx` manifest at all; v1.1.1 got past that and died on a stale pin `--locked` would not reconcile.

    A lockfile records a version for every package it resolved, and two of those
    are ours: `blast-search`, whose manifest the release bot bumps and whose lock entry
