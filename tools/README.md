...
records for `blast-search` falls behind the moment release-please bumps
...
repository-relative path is a 404 under `pypi.org/project/blast-search/`, and on
...
`blast --version` and the `--schema` manifest answer with — Rust reads
...
This package composes three siblings, and each is a **different axis**:
`irregex`, `relate`, and `gist` version on their own schedules and are pinned as
dependencies, never mirrored here. The face used to print the engine's version
as its own — 1.0.0 against a package at 0.1.0 — which is the drift this
